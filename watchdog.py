...
def log_analyzer(file_path):
  try:
    with open(file_path, 'r') as file:
      next(file)
      for line_number, log_entry in enumerate(file, start=2):
        if detect_sqli(log_entry.strip()):
          print(f"Potential SQL injection detected at line {line_number}: {log_entry.strip()}")
